...
export default function SupportPage() {
  ...
  useEffect(() => { if (auth) { loadList(); loadStats() } }, [auth])
  useEffect(() => { if (selected) loadDetail(selected) }, [selected])

  function loadStats() {
    fetch('/api/admin/tickets?action=stats').then(r=>r.json()).then(d=>setStats(d.stats||null)).catch(()=>{})
  }
  ...
  async function sendReply() {
    ...
    try {
      ...
      if (d.success) { setReply(''); setIsInternal(false); loadDetail(selected); loadList(); loadStats() }
      ...
      <AdminShell activeSection="support" breadcrumb={[{ label:'Support' }]}>
        ...
        {stats && stats.total > 0 && (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:14, marginBottom:24 }}>
            <Card padding={18}>
              <div style={{ fontSize:11, fontWeight:600, color:tokens.textDim, textTransform:'uppercase', letterSpacing:'.06em', marginBottom:6 }}>⏱ Temps réponse moyen</div>
              <div style={{ fontFamily:tokens.fontDisplay, fontSize:24, fontWeight:700, color: stats.avg_response_hours < 4 ? tokens.success : stats.avg_response_hours < 24 ? tokens.warning : tokens.danger }}>
                {stats.avg_response_hours < 1 ? Math.round(stats.avg_response_hours*60)+' min' : stats.avg_response_hours < 48 ? stats.avg_response_hours+' h' : Math.round(stats.avg_response_hours/24)+' j'}
              </div>
              <div style={{ fontSize:11, color:tokens.textDim, marginTop:4 }}>Sur {stats.with_response} ticket{stats.with_response>1?'s':''} avec réponse</div>
            </Card>
            <Card padding={18}>
              <div style={{ fontSize:11, fontWeight:600, color:tokens.textDim, textTransform:'uppercase', letterSpacing:'.06em', marginBottom:6 }}>✅ Taux de résolution</div>
              <div style={{ fontFamily:tokens.fontDisplay, fontSize:24, fontWeight:700, color: stats.taux_resolution > 70 ? tokens.success : stats.taux_resolution > 40 ? tokens.warning : tokens.danger }}>
                {stats.taux_resolution}%
              </div>
              <div style={{ fontSize:11, color:tokens.textDim, marginTop:4 }}>{stats.resolus + stats.fermes} sur {stats.total} tickets</div>
            </Card>
            <Card padding={18}>
              <div style={{ fontSize:11, fontWeight:600, color:tokens.textDim, textTransform:'uppercase', letterSpacing:'.06em', marginBottom:6 }}>🎯 À traiter</div>
              <div style={{ fontFamily:tokens.fontDisplay, fontSize:24, fontWeight:700, color: (stats.ouverts + stats.en_cours) === 0 ? tokens.success : tokens.warning }}>
                {stats.ouverts + stats.en_cours}
              </div>
              <div style={{ fontSize:11, color:tokens.textDim, marginTop:4 }}>Tickets ouverts + en cours</div>
            </Card>
            <Card padding={18}>
              <div style={{ fontSize:11, fontWeight:600, color:tokens.textDim, textTransform:'uppercase', letterSpacing:'.06em', marginBottom:6 }}>🚨 Priorité urgente</div>
              <div style={{ fontFamily:tokens.fontDisplay, fontSize:24, fontWeight:700, color: stats.urgents === 0 ? tokens.success : tokens.danger }}>
                {stats.urgents}
              </div>
              <div style={{ fontSize:11, color:tokens.textDim, marginTop:4 }}>À traiter en priorité</div>
            </Card>
          </div>
        )}
