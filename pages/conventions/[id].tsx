...
export default function ConventionView() {
  ...
  async function generatePDF() {
    ...
    try {
      ...
      doc.setFontSize(10)
      doc.setFont('helvetica','normal')
      doc.text(`Prix HT : ${convention.prix_ht?.toLocaleString('fr-FR')}€`, 20, y+6)
      doc.text(`TVA (${convention.tva_taux}%) : ${convention.tva_montant?.toLocaleString('fr-FR')}€`, 20, y+11)
      doc.setFont('helvetica','bold')
      doc.setFontSize(11)
      doc.text(`Total TTC : ${convention.prix_ttc?.toLocaleString('fr-FR')}€`, 20, y+18)
      doc.setFontSize(9)
      doc.setFont('helvetica','normal')
      doc.text(`Financement : ${convention.financement?.toUpperCase()} — Paiement : ${convention.modalites_paiement}`, 105, y+11, {align:'center'})
      y += 28
      ...
      <div style={{ minHeight:'100vh', background:'#050c1a', color:'#e2e8f0', fontFamily:'DM Sans,system-ui', padding:'24px' }}>
        <div style={{ maxWidth:900, margin:'0 auto' }}>
          ...
          <div style={{ background:'#fff', color:'#0f172a', borderRadius:14, padding:32, fontFamily:'Helvetica, Arial, sans-serif' }}>
            ...
            <div style={{ background:'#f0fdf4', padding:16, borderRadius:8, marginBottom:14, border:'1px solid #86efac' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:14, textAlign:'center' }}>
                <div>
                  ...
                  <div style={{ fontSize:18, fontWeight:700, color:'#0f172a' }}>{convention.prix_ht?.toLocaleString('fr-FR')}€</div>
                </div>
                <div>
                  <div style={{ fontSize:11, color:'#64748b' }}>TVA ({convention.tva_taux}%)</div>
                  <div style={{ fontSize:18, fontWeight:700, color:'#f59e0b' }}>{convention.tva_montant?.toLocaleString('fr-FR')}€</div>
                </div>
                <div>
                  <div style={{ fontSize:11, color:'#64748b' }}>Total TTC</div>
                  <div style={{ fontSize:18, fontWeight:700, color:'#10b981' }}>{convention.prix_ttc?.toLocaleString('fr-FR')}€</div>
                </div>
              </div>
              <div style={{ fontSize:11, color:'#64748b', textAlign:'center', marginTop:8 }}>Financement : {convention.financement?.toUpperCase()} — {convention.modalites_paiement}</div>
            </div>
