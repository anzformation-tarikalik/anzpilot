...
export default function SupportOFPage() {
  ...
  async function toggleTicket(newAction: 'close' | 'reopen') {
    if (!selected) return
    const confirmMsg = newAction === 'close' ? 'Marquer ce ticket comme résolu ?' : 'Rouvrir ce ticket ?'
    if (!confirm(confirmMsg)) return
    try {
      const res = await fetch('/api/of/tickets', {
        method:'PUT', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ id: selected, email, action: newAction })
      })
      const d = await res.json()
      if (d.success) { loadDetail(selected); loadTickets(email) }
      else alert('Erreur: '+(d.error||''))
    } catch(e:any) { alert(e.message) }
  }
              ...
              <div style={{ background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.08)', borderRadius:14, overflow:'hidden' }}>
                ...
                : (
                  <>
                    ...
                    <div style={{ padding:'10px 20px', borderTop:'1px solid rgba(255,255,255,.06)', background:'#050c1a', display:'flex', justifyContent:'flex-end', gap:8 }}>
                      {detail.ticket.statut === 'resolu' || detail.ticket.statut === 'ferme' ? (
                        <button onClick={()=>toggleTicket('reopen')} style={{ padding:'6px 12px', borderRadius:8, border:'1px solid rgba(14,165,233,.3)', background:'rgba(14,165,233,.1)', color:'#0ea5e9', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'DM Sans,system-ui' }}>🔄 Rouvrir ce ticket</button>
                      ) : (
                        <button onClick={()=>toggleTicket('close')} style={{ padding:'6px 12px', borderRadius:8, border:'1px solid rgba(16,185,129,.3)', background:'rgba(16,185,129,.1)', color:'#10b981', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'DM Sans,system-ui' }}>✅ Marquer comme résolu</button>
                      )}
                    </div>

                    {detail.ticket.statut !== 'ferme' && detail.ticket.statut !== 'resolu' && (
