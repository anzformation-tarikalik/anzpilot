...
export default function FactureView() {
  ...
  async function genPDF() {
    ...
    try {
      ...
      // Totaux
      doc.setDrawColor(200,200,200); doc.line(120, y, 196, y); y += 6
      doc.text('Sous-total HT:', 130, y); doc.text(`${(f.prix_ht||0).toLocaleString('fr-FR')}€`, 196, y, {align:'right'}); y += 5
      doc.text(`TVA (${f.tva_taux||0}%):`, 130, y); doc.text(`${(f.tva_montant||0).toLocaleString('fr-FR')}€`, 196, y, {align:'right'}); y += 8
      doc.setFillColor(16,185,129); doc.rect(120, y-4, 76, 10, 'F'); doc.setTextColor(255,255,255)
      doc.setFont('helvetica','bold'); doc.setFontSize(11)
      doc.text('TOTAL TTC:', 124, y+2); doc.text(`${(f.prix_ttc||0).toLocaleString('fr-FR')}€`, 192, y+2, {align:'right'})
      y += 16
        ...
        <div style={{ maxWidth:900, margin:'0 auto' }}>
          ...
          <div style={{ background:'#fff', color:'#0f172a', borderRadius:14, padding:32, fontFamily:'Helvetica, Arial, sans-serif' }}>
            ...
            <div style={{ display:'flex', justifyContent:'flex-end' }}>
              <div style={{ width:300 }}>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', fontSize:13 }}><span>Sous-total HT</span><span>{f.prix_ht?.toLocaleString('fr-FR')}€</span></div>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', fontSize:13, borderBottom:'1px solid #e2e8f0' }}><span>TVA ({f.tva_taux}%)</span><span>{f.tva_montant?.toLocaleString('fr-FR')}€</span></div>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'10px 14px', marginTop:8, background:'#10b981', color:'#fff', borderRadius:6, fontSize:15, fontWeight:700 }}><span>TOTAL TTC</span><span>{f.prix_ttc?.toLocaleString('fr-FR')}€</span></div>
              </div>
            </div>
