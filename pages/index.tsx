...
const empty: Modele = { id:'', titre:'', objectifs:'', programme:'', modalites:'présentiel', duree_heures:0, lieu:'', prix_ht:0, tva_taux:20, categorie:'' }
...
export default function Catalogue() {
  ...
  const totalCA = modeles.reduce((s,m) => s+(m.prix_ht||0)*1.2, 0)
        ...
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:14, marginBottom:24 }}>
            {[
              ...
              ['💰','CA potentiel TTC',Math.round(totalCA).toLocaleString('fr-FR')+'€','#8b5cf6'],
                ...
                <div key={m.id} style={{ background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.08)', borderRadius:14, padding:18, display:'flex', flexDirection:'column', gap:8 }}>
                  ...
                  <div style={{ marginTop:'auto', display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:10, borderTop:'1px solid rgba(255,255,255,.05)' }}>
                    <div>
                      ...
                      <div style={{ fontSize:10, color:'#64748b' }}>HT · TVA {m.tva_taux||0}%</div>
                    </div>
          ...
          <div onClick={()=>setShowForm(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100, padding:20 }}>
            <div onClick={e=>e.stopPropagation()} style={{ background:'#0a1628', border:'1px solid rgba(255,255,255,.1)', borderRadius:14, padding:24, maxWidth:650, width:'100%', maxHeight:'90vh', overflowY:'auto' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
                ...
                <div><label style={labelStyle}>Prix HT (€)</label><input type="number" value={editing.prix_ht} onChange={e=>update('prix_ht', parseFloat(e.target.value)||0)} placeholder="2000" style={inputStyle}/></div>
                <div><label style={labelStyle}>TVA (%)</label><input type="number" value={editing.tva_taux} onChange={e=>update('tva_taux', parseFloat(e.target.value)||0)} style={inputStyle}/></div>
              ...
              {editing.prix_ht > 0 && (
                <div style={{ padding:12, background:'rgba(14,165,233,.05)', border:'1px solid rgba(14,165,233,.2)', borderRadius:9, marginBottom:14, fontSize:12 }}>
                  <span style={{ color:'#64748b' }}>Prix TTC : </span>
                  <strong style={{ color:'#10b981' }}>{(editing.prix_ht * (1 + (editing.tva_taux||0)/100)).toLocaleString('fr-FR', {maximumFractionDigits:2})}€</strong>
                </div>
              )}
