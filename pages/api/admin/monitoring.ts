...
async function pingResend(): Promise<ServiceStatus> {
  ...
  try {
    ...
    const latency = Date.now() - t0
    if (!res.ok) return { name:'Resend', slug:'resend', icon:'📧', status:'down', latency_ms:latency, error:'HTTP '+res.status }
    const data = await res.json()
    const domain = (data.data||[]).find((d:any) => d.name === (EMAIL_FROM.split('@')[1] || 'anzpilot.com'))
    const verified = domain?.status === 'verified'
    return {
      name:'Resend', slug:'resend', icon:'📧',
      status: verified ? (latency<500?'up':'degraded') : 'degraded',
      latency_ms:latency,
      details: verified ? `Domaine ${domain.name} vérifié` : (domain ? `Statut: ${domain.status}` : 'Aucun domaine')
    }
  } catch (e: any) {
