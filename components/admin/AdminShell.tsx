...
const MENU = [
  ...
  },
  {
    ...
    items: [
      ...
      { id:'abonnements', icon:'💳', label:'Abonnements', href:'/admin/billing' },
  ...
  },
  {
    ...
    items: [
      ...
      { id:'monitoring', icon:'🩺', label:'Monitoring', href:'/admin/monitoring' },
    ]
  },
  {
    title:'CONFIGURATION',
    items: [
      ...
      { id:'communications', icon:'🔔', label:'Communications', href:'/admin/communications' },
      { id:'integrations', icon:'🔌', label:'Intégrations', href:'/admin/integrations' },
    ]
  },
  {
    title:'SÉCURITÉ',
...
export default function AdminShell({ children, activeSection='dashboard', breadcrumb }: AdminShellProps) {
  ...
  useEffect(() => {
    function loadUnread() {
      fetch('/api/notifications/count?type=admin').then(r=>r.json()).then(d=>{
        setTicketsUnread(d.tickets_unread || 0)
        setUrgentUnread(d.urgent || 0)
      }).catch(()=>{})
    }
    loadUnread()
    const id = setInterval(loadUnread, 30000) // refresh 30s
          ...
          {MENU.map((section, si) => (
            <div key={si} style={{ marginBottom: section.title?18:0 }}>
              ...
              {section.items.map(item => {
                ...
                const badgeColor = urgentUnread > 0 ? tokens.danger : tokens.primary
                return (
                  <Link key={item.id} href={item.href} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 10px', borderRadius:8, color: isActive ? tokens.primary : tokens.textMuted, background: isActive ? 'rgba(14,165,233,.1)' : 'transparent', textDecoration:'none', fontSize:13, fontWeight:500, marginBottom:2, transition:'all .15s', position:'relative' }}>
                    <span style={{ fontSize:16, width:20, textAlign:'center', flexShrink:0, position:'relative' }}>
                      {item.icon}
                      {collapsed && showBadge && <span style={{ position:'absolute', top:-4, right:-6, minWidth:14, height:14, borderRadius:7, background:badgeColor, color:'#fff', fontSize:9, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', padding:'0 3px' }}>{ticketsUnread}</span>}
                    </span>
                    {!collapsed && <span style={{ flex:1 }}>{item.label}</span>}
                    {!collapsed && showBadge && <span style={{ fontSize:10, fontWeight:700, padding:'2px 7px', borderRadius:10, background:badgeColor, color:'#fff', minWidth:20, textAlign:'center' }}>{ticketsUnread}</span>}
